...
const register = () => {
    ...
    authenticate(ID, email, password)
};
function updateLog() {
    input = document.getElementById("log").value
    console.log(userId)
    console.log("//////////////////", userId)
    database.ref('users/nobita/LOG').set({
        log: input
    })
}
...
const authenticate = (ID, email, password) => {
    const auth = firebase.auth();
...
// auth.onAuthStateChanged((firebaseUser) => {
//     if (firebaseUser) {
//         console.log("uuuuuuuuuuuuuuuuu", firebaseUser)
//         showHomepage();
//     }
// });
